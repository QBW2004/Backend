        ...
        public Msg Post([FromBody] ExChangeModel model)
        {
            lock (this)
            {
                ...
                try
                {
                    ...
                    msg1 = new B_Users().ExChangeCoin(playerId, coins);//修改用户金币
                    if (msg1.code == 10000)
                    {
                        int val1 = new B_Admin().ExChangeCoin(usr.AGENCY, coins);//修改代理金币
                        ReturnModel returnModel = new ReturnModel();
                        M_Users usr1 = new B_Users().GetSingle(new M_Users { ID = model.playerId });
                        returnModel.coins = usr1.COINS;
                        ...
                        //【旧版本】通知所有后台账号
                        hub.Clients.All.sayHello($"用户【{model.playerId}】兑换，请及时处理订单。");
                    }
                }
                catch (Exception ex)
                {
